...
##Patients are included only if they had random treatment assignment, non-missing 30-day mRS score, 
##and non-missing baseline NIHSS score

##########################################################################################################
##Gather the data for 30-day mRS score##
##########################################################################################################
...
##All inclusion criteria have been met except non-missing 30-day mRS score and non-missing NIHSS score

##We only need mRSData and baselineData right now
rm(clotData)

##Also, we want to focus on the data at 30-days
mrs30 <- subset(mrsData, Follow_up_Visit == 30) 
rm(mrsData)
length(unique(mrs30$patientName)) ##96

##Merge mrs30 with baselineData
data <- merge(baselineData, mrs30, by = "patientName")
...
rm(drops, mrs30, baselineData)

##Only include people with baseline NIHSS and 30-day mRS score
data$patientName[is.na(data$Enrollment_NIHSS_Total)] 
data <- subset(data, !is.na(Enrollment_NIHSS_Total))
nrow(data) ##95

sum(is.na(data$rankin_score)) ##3 people missing rankin_score
data$patientName[is.na(data$rankin_score)] 
data <- subset(data, !is.na(rankin_score)) 
nrow(data) ##92

##There will be 92 people total in this analysis

control <- subset(data, Group_Assigned.x == "Medical") ##39
treatment <- subset(data, Group_Assigned.x == "Surgical") ##53

YC <- control$rankin_score 
YT <- treatment$rankin_score

control1 <- subset(control, Enrollment_NIHSS_Total <= 20) ##18
control2 <- subset(control, Enrollment_NIHSS_Total > 20) ##21
tmt1 <- subset(treatment, Enrollment_NIHSS_Total <= 20) ##21
tmt2 <- subset(treatment, Enrollment_NIHSS_Total > 20) ##32
