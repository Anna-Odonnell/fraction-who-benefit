...
for (m in 1:nsim){
  ...
  templ <- unique(boot.out$t[,1])
  tempu <- unique(boot.out$t[,2])
    
  if(max(templ)-min(templ)>10^(-10)){
    LB.CI_perc[m,] <- boot.ci(boot.out, index = 1, conf=0.95, type="perc")$perc[4:5] 
  } else {
    print("lower bounds of bootstrap replicates are within 10e-10")
    LB.CI_perc[m,] <- c(min(templ),max(templ))
  }
    
  if(max(tempu)-min(tempu)>10^(-10)){
    UB.CI_perc[m,] <- boot.ci(boot.out, index = 2, conf=0.95, type="perc")$perc[4:5]
  } else{
    print("upper bounds of bootstrap replicates are within 10e-10")
    UB.CI_perc[m,] <- c(min(tempu),max(tempu))
  }
}
...
save(bounds, LB.CI_perc, UB.CI_perc, file=paste("clotVol100_noRes","-","seed", seed, "-", format(Sys.time(), "%Y%m%d-%H%M"),".Rdata", sep=""))
