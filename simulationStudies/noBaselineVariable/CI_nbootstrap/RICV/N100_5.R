...
for (m in 1:nsim){
  ...
  LB.CI_perc[m,] <- c(quantile(boot.out$t[,1], probs = c(.025,0.975)))
  UB.CI_perc[m,] <- c(quantile(boot.out$t[,2], probs = c(.025,0.975)))
}


save(bounds, LB.CI_perc, UB.CI_perc, file="N100_5.Rdata")
