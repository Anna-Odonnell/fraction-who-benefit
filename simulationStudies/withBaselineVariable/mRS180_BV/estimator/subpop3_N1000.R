...
pT3.1 <- FT3.1 - c(0,FT3.1[-length(FT3.1)])
pT3.2 <- FT3.2 - c(0,FT3.2[-length(FT3.2)])
pT3.3 <- FT3.3 - c(0,FT3.3[-length(FT3.3)])

pC3.1 <- FC3.1 - c(0,FC3.1[-length(FC3.1)])
pC3.2 <- FC3.2 - c(0,FC3.2[-length(FC3.2)])
pC3.3 <- FC3.3 - c(0,FC3.3[-length(FC3.3)])
#############################################################################
...
for (m in 1:nsim){
  ...
  data$X <- sample(1:K, size = nrow(data), replace = TRUE, prob = p3)
  
  data$Y[data$A == "Medical" & data$X == 1] <- sample(ordinalScale, size = sum(data$A == "Medical" & data$X == 1), replace = TRUE, prob = pC3.1)
  data$Y[data$A == "Medical" & data$X == 2] <- sample(ordinalScale, size = sum(data$A == "Medical" & data$X == 2), replace = TRUE, prob = pC3.2)
  data$Y[data$A == "Medical" & data$X == 3] <- sample(ordinalScale, size = sum(data$A == "Medical" & data$X == 3), replace = TRUE, prob = pC3.3)
  
  data$Y[data$A == "Surgical" & data$X == 1] <- sample(ordinalScale, size = sum(data$A == "Surgical" & data$X == 1), replace = TRUE, prob = pT3.1)
  data$Y[data$A == "Surgical" & data$X == 2] <- sample(ordinalScale, size = sum(data$A == "Surgical" & data$X == 2), replace = TRUE, prob = pT3.2)
  data$Y[data$A == "Surgical" & data$X == 3] <- sample(ordinalScale, size = sum(data$A == "Surgical" & data$X == 3), replace = TRUE, prob = pT3.3)
  ...
  bounds[m,] <- mean(data$X == 1) * bounds1 + mean(data$X == 2) * bounds2 + mean(data$X == 3) * bounds3
}
...
save(bounds, file="subpop3_N1000.Rdata")
