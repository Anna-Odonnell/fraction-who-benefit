function [l,u,lflag, uflag] = boundsNoCov_resBen(YT, YC, d)
%This function calculates the bounds on the fraction who benefit, with a restriction on benefit but no
%baseline variables.
...
%d applies to the restriction on benefit, which is Benefit = YT - YC <= d
...
%number of variables
varCount = mT * mC;
%number of constraints from marginal pmf's and restriction on benefit
constCount = mT + mC + 1; 
...
beq = [margPMF_T; margPMF_C; 0]; %0 is appended for restriction on benefit

%Aeq

%i1,i2,j1,j2 are for marginal pmf's
%i3 and j3 are for restriction on benefit
j3 = zeros(mC, mT);
for r = 1:mC
    for c = 1:mT
        if (YT_sort(c) - YC_sort(r)) > d
            j3(r,c) = 1;
        end
    end
end
