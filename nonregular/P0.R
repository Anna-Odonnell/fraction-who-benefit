...
##Compute the distribution of sqrt(n)[psiHAT_l - psi_l(P_0)], under P_0, 
...
for (i in 1:nsim){
  data <- data.frame(A = sample(0:1, size = n, replace = TRUE), Y = sample(1:2, size = n, replace = TRUE))
  result100[i] <- boundsNoCov_res(ordinalScale, data$Y[data$A == 1], data$Y[data$A == 0], maxBen, maxHarm)[1]
}
result100 <- result100*sqrt(n)
...
result10000 <- result10000*sqrt(n)
...
resultMillion <- resultMillion*sqrt(n)

########################################################################
##Save results##
########################################################################
save(result100, result1000, result10000, resultMillion, file = "distributions_P0.Rdata")
